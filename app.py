...
# 🔍 SCOPE 수정: Points와 Mileage 권한을 모두 포함하여 안정성 확보
    ...
    if uploaded_file:
        try:
            ...
            if 'summary_df' in st.session_state:
                ...
                with b2:
                    # 🚀 [확인 완료] API Endpoint: /api/v2/admin/points 호출
...
# (기록 조회 및 삭제 로직은 기존 기능 유지)
    ...
    try:
        ...
        if not raw_df.empty:
            out = io.BytesIO()
            with pd.ExcelWriter(out, engine='xlsxwriter') as w: raw_df.to_excel(w, index=False)
