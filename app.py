...
if menu == "적립금 지급하기":
    ...
    # --- 🚨 [수정된 인증 로직] 에러 발생 시 상세 내용을 출력합니다. ---
    if "code" in st.query_params and "access_token" not in st.session_state:
        auth_code = st.query_params["code"]
        with st.spinner("🔄 카페24 인증을 시도 중입니다..."):
            token, error_detail = get_access_token(auth_code)
            if token:
                st.session_state["access_token"] = token
                st.query_params.clear()
                st.rerun()
            else:
                st.error("❌ 카페24 연동에 실패했습니다.")
                st.expander("상세 에러 내용 보기").code(error_detail)
                st.info("💡 팁: Client ID/Secret이 새 쇼핑몰 정보와 일치하는지, Redirect URI가 정확한지 확인해 보세요.")
